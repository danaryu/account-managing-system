...
public interface AccountHistoryRepository extends JpaRepository<AccountHistory, Long> {
    ...
    @Query("select distinct new com.kkopaysec.assignment.banking.dto.DepositByMemberAccount(SUM(ah.amount), ah.account.accountNumber) " +
            "from AccountHistory as ah " +
            "inner join ah.account " +
            "where ah.account.accountHolder = :accountHolder " +
            "and ah.accountStatus = :accountStatus " +
            "group by ah.account.accountNumber, ah.accountStatus")
    List<DepositByMemberAccount> findAllDepositByMemberAccount(@Param("accountHolder") Member accountHolder, @Param("accountStatus") AccountStatus accountStatus);
